                                                ...
                                                JOIN ruangan d ON a.kode_ruangan=d.kode_ruangan
                                                  ...
                                                  where a.kode_kelas='$_GET[kelas]' 
                                                    AND a.id_tahun_akademik='$_GET[tahun]' 
                                                      AND b.kode_kurikulum='$kurikulum[kode_kurikulum]' ORDER BY a.hari ASC");
            }
            $no = 1;
...
<?php
  ...
  while ($r = mysqli_fetch_array($tampil)) {
    ...
    if ($_SESSION[level] == 'kepala') {
      ...
      echo "<td><a class='btn btn-primary btn-xs' title='Lihat Soal' href='index.php?view=soal&act=semuasoal&jdwl=$_GET[jdwl]&idsoal=$r[id_quiz_ujian]&id=$_GET[id]&kd=$_GET[kd]'><span class='glyphicon glyphicon-search'></span> Lihat Soal</a>
                                          ...
                                          <a class='btn btn-danger btn-xs alert_notif' title='Delete Bahan dan Tugas' href='index.php?view=soal&act=listsoal&jdwl=$_GET[jdwl]&id=$_GET[id]&kd=$_GET[kd]&hapus=$r[id_quiz_ujian]'><span class='glyphicon glyphicon-remove'></span></a></td>";
    }
    echo "</tr>";
    $no++;
  }
  ...
  echo "</select>
              ...
              <div class='box-footer'>
                    ...
                    <a href='javascript:window.history.back();'><button class='btn btn-default pull-right'>Cancel</button></a>
  ...
  if (isset($_GET[editessai])) {
    mysqli_query($koneksi, "UPDATE pertanyaan_essai SET pertanyaan_essai='$_POST[a]' where id_pertanyaan_essai='$_GET[idsoal]'");
    echo "<script>document.location='index.php?view=soal&act=semuasoal&jdwl=$_GET[jdwl]&idsoal=$_GET[idsoal]&id=$_GET[id]&kd=$_GET[kd]';</script>";
  }

  if (isset($_GET[editobjektif])) {
    mysqli_query($koneksi, "UPDATE pertanyaan_objektif SET id_pertanyaan_objektif='$_POST[editobjektif]'");
    echo "<script>document.location='index.php?view=soal&act=semuasoal&jdwl=$_GET[jdwl]&idsoal=$_GET[idsoal]&id=$_GET[id]&kd=$_GET[kd]';</script>";
  }
  ...
  $essai = mysqli_query($koneksi, "SELECT * FROM `pertanyaan_essai` where id_quiz_ujian='$_GET[idsoal]' ORDER BY id_pertanyaan_essai ASC");
  $no = 1;
  while ($k = mysqli_fetch_array($essai)) {
    echo "<tr>
                            <td>$no</td>
                            <td>$k[pertanyaan_essai]</td>
                            <td style='width:80px'><center><a  class='btn btn-danger btn-xs alert_notif' href='index.php?view=soal&act=semuasoal&jdwl=$_GET[jdwl]&idsoal=$_GET[idsoal]&id=$_GET[id]&kd=$_GET[kd]&deleteessai=$k[id_pertanyaan_essai]'><span class='glyphicon glyphicon-remove'></span></a></center></td>
                            </tr>";
    $no++;
  }
  echo "</table>
  ...
  $objektif = mysqli_query($koneksi, "SELECT * FROM `pertanyaan_objektif` where id_quiz_ujian='$_GET[idsoal]' ORDER BY id_pertanyaan_objektif ASC");
  $noo = 1;
  ...
  echo "</select>
    ...
    $kunci = array("$ko[jawab_a]", "$ko[jawab_b]", "$ko[jawab_c]", "$ko[jawab_d]", "$ko[jawab_e]");
    echo "<form action='' method='post'>";
    echo "Pilih jawaban:<br>";
      for($i=0; $i<5; $i++){
        echo "<input type='radio' name='$noo' value='".$kunci[$i]."'>".$kunci[$i]."<br>";
      }
      echo "Pilih kunci jawaban:<br>";
      // Membuat dropdown untuk kunci jawaban
      echo "<select name='g'>";
        for($i=0; $i<5; $i++){
          echo "<option value='".$kunci[$i]."'>".$kunci[$i]."</option>";
        }
      echo "</select>"; */
      
    echo "<div class='btn btn-default btn-xs btn-block'>Kunci Jawaban : $ko[kunci_jawaban]
    
    </div>";
                            echo "</td>
                            <td style='width:60px'><center><a class='btn btn-danger btn-xs alert_notif' href='index.php?view=soal&act=semuasoal&jdwl=$_GET[jdwl]&idsoal=$_GET[idsoal]&id=$_GET[id]&kd=$_GET[kd]&deleteobjektif=$ko[id_pertanyaan_objektif]'><span class='glyphicon glyphicon-remove'></span></a></td>
                            </tr>";
    $noo++;
  }
  echo "</table>
  ...
  $objek = mysqli_query($koneksi, "SELECT * FROM `pertanyaan_objektif` where id_quiz_ujian='$_GET[idsoal]' ORDER BY id_pertanyaan_objektif ASC");
      ...
      <div class="box-header">
        ...
        <form style='margin-right:5px; margin-top:0px' class='pull-right' action='' method='GET'>
          ...
          <select name='tahun' style='padding:4px'>
            ...
            echo "<option value=''>- Pilih Tahun Akademik -</option>";
            $tahun = mysqli_query($koneksi, "SELECT * FROM tahun_akademik");
            $guru = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM guru where nip = '$_SESSION[id]'"));
            while ($k = mysqli_fetch_array($tahun)) {
              if ($_GET[tahun] == $k[id_tahun_akademik]) {
                echo "<option value='$k[id_tahun_akademik]' selected>$k[nama_tahun]</option>";
              } else {
                                                ...
                                                JOIN kelas e ON a.kode_kelas=e.kode_kelas 
                                                  where a.nip='$guru[nik]' 
                                                    AND b.kode_kurikulum='$kurikulum[kode_kurikulum]'
                                                      AND a.id_tahun_akademik LIKE '%" . date('Y') . "%' ORDER BY a.hari ASC");
            }
            $no = 1;
            while ($r = mysqli_fetch_array($tampil)) {
              $total = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM quiz_ujian where kodejdwl='$r[kodejdwl]'"));
...
?>
  <div class="col-xs-12">
    <div class="box">
      <div class="box-header">
        <h3 class="box-title"><?php if (isset($_GET[kelas])) {
                                echo "Quiz dan Ujian Online";
                              } else {
