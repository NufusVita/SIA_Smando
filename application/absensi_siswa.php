            ...
            while ($r = mysqli_fetch_array($tampil)) {
              ...
              if ($_SESSION[level] != 'kepala') {
                echo "<td style='width:70px !important'><center>
                                <a class='btn btn-success btn-xs' title='Tampil Absensi' href='index.php?view=absensiswa&act=tampilabsen&id=$r[kode_kelas]&kd=$r[kode_pelajaran]&jdwl=$r[kodejdwl]'><span class='glyphicon glyphicon-th'></span>Tampil Absensi</a>
                              </center></td>";
              }
              echo "</tr>";
              $no++;
              ...
              $tampil = mysqli_query($koneksi, "SELECT a.*, e.nama_kelas, b.namamatapelajaran, b.kode_pelajaran, c.nama_guru, d.nama_ruangan FROM jadwal_pelajaran a 
                                            JOIN mata_pelajaran b ON a.kode_pelajaran=b.kode_pelajaran
                                              JOIN guru c ON a.nip=c.nik 
                                                JOIN ruangan d ON a.kode_ruangan=d.kode_ruangan
                                                  JOIN kelas e ON a.kode_kelas=e.kode_kelas 
                                                  where a.nip='$guru[nik]' AND a.id_tahun_akademik='$_GET[tahun]' ORDER BY a.hari ASC");
            } else {
              $tampil = mysqli_query($koneksi, "SELECT a.*, e.nama_kelas, b.namamatapelajaran, b.kode_pelajaran, c.nama_guru, d.nama_ruangan FROM jadwal_pelajaran a 
                                            JOIN mata_pelajaran b ON a.kode_pelajaran=b.kode_pelajaran
                                              JOIN guru c ON a.nip=c.nik 
                                                JOIN ruangan d ON a.kode_ruangan=d.kode_ruangan
                                                JOIN kelas e ON a.kode_kelas=e.kode_kelas 
                                                  where a.nip='$guru[nik]' AND a.id_tahun_akademik LIKE '%" . date('Y') . "%' ORDER BY a.hari ASC");
            }
            $no = 1;
            while ($r = mysqli_fetch_array($tampil)) {
              echo "<tr><td>$no</td>
                              ...
                              <td>$r[nama_ruangan]</td>
                              <td>$r[id_tahun_akademik]</td>
                              <td><a class='btn btn-success btn-xs' title='Tampil List Absensi' href='index.php?view=absensiswa&act=tampilabsen&id=$r[kode_kelas]&kd=$r[kode_pelajaran]'><span class='glyphicon glyphicon-th'></span> Tampilkan Absensi</a></td>
                          </tr>";
              $no++;
            }
            ?>
...
<?php
  ...
  $tampil = mysqli_query($koneksi, "SELECT * FROM jadwal_pelajaran a 
                                            JOIN mata_pelajaran b ON a.kodepelajaran=b.kodepelajaran
                                              JOIN guru c ON a.nip=c.nik 
                                                JOIN kelas d ON a.kodekelas=d.kodekelas where a.kodekelas='$iden[kodekelas]' AND a.semester='1' ORDER BY a.hari ASC");
  $no = 1;
  while ($r = mysqli_fetch_array($tampil)) {
    echo "<tr><td>$no</td>
  ...
  $tampil = mysqli_query($koneksi, "SELECT * FROM jadwal_pelajaran a 
                                            JOIN mata_pelajaran b ON a.kodepelajaran=b.kodepelajaran
                                              JOIN guru c ON a.nip=c.nik 
                                                JOIN kelas d ON a.kodekelas=d.kodekelas where a.kodekelas='$iden[kodekelas]' AND a.semester='2' ORDER BY a.hari ASC");
  $no = 1;
  while ($r = mysqli_fetch_array($tampil)) {
