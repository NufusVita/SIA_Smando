        ...
        for ($ia=1; $ia<=$jumls; $ia++){
          ...
          if ($a != '' OR $b != ''){
            ...
            if ($cek >= '1'){
              ...
              mysqli_query($koneksi,"INSERT INTO nilai_uts (kodejdwl, nisn, angka_pengetahuan, deskripsi_pengetahuan, angka_keterampilan, deskripsi_keterampilan, waktu_input_uts) VALUES('$_POST[jdwl]','$nisn','$a','','$b','','".date('Y-m-d H:i:s')."')");
            }
          }
        }
              ...
              <table class='table table-condensed table-hover'>
                ...
                <div class='col-md-12'>
                  <table class='table table-bordered table-striped'>
                    ...
                    // $nilaiKeterampilan = 0;
                    $no = 1;
                    $tampil = mysqli_query($koneksi,"SELECT * FROM siswa where kode_kelas='$_GET[id]' ORDER BY id_siswa ASC");
                    while($r=mysqli_fetch_assoc($tampil)){
                      $n = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM nilai_uts where nisn='$r[nisn]' AND kodejdwl='$_GET[jdwl]'"));
                      $cekpredikat = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM predikat"));

                      // var_dump($r['nisn']);
                      // die();

                      $nilaiPengetahuan = $n['angka_pengetahuan']?$n['angka_pengetahuan']:0;
                      $nilaiKeterampilan = $n['angka_keterampilan']?$n['angka_keterampilan']:0;

                      if ($cekpredikat >= 1){
                        $grade1 = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM predikat where ($nilaiPengetahuan >= nilai_a) AND ($nilaiPengetahuan <= nilai_b)"));
                        $grade2 = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM predikat where ($nilaiKeterampilan >= nilai_a) AND ($nilaiKeterampilan <= nilai_b)"));
                      }else{
                        $grade1 = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM predikat where ($nilaiPengetahuan >= nilai_a) AND ($nilaiPengetahuan <= nilai_b)"));
                        $grade2 = mysqli_fetch_array(mysqli_query($koneksi,"SELECT * FROM predikat where ($nilaiKeterampilan >= nilai_a) AND ($nilaiKeterampilan <= nilai_b)"));
                      }
                        echo "<tr>
                              ...
                              <td align=center><input type='number' name='a".$no."' value='$nilaiPengetahuan' style='width:90px; text-align:center; padding:0px' placeholder='-'></td>
                                        ...
                                        <td align=center><input type='number' name='b".$no."' value='$nilaiKeterampilan' style='width:90px; text-align:center; padding:0px' placeholder='-'></td>
