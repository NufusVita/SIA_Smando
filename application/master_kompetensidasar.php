...
<?php
  ...
  $sql = "SELECT * FROM guru where nik='$d[nip]'";
  $nama_guru = "Belum Memilih";
  ...
  echo "</tr>
                    ...
                    <tbody>";
  $tampil = mysqli_query($koneksi, "SELECT * FROM kompetensi_dasar z JOIN jadwal_pelajaran a ON z.kodejdwl=a.kodejdwl JOIN kelas b ON a.kode_kelas=b.kode_kelas JOIN mata_pelajaran c ON a.kode_pelajaran=c.kode_pelajaran where a.kodejdwl='$_GET[id]' ORDER BY z.id_kompetensi_dasar ASC");
  $no = 1;
  while ($r = mysqli_fetch_array($tampil)) {
    echo "<tr><td>$no</td>
                              <td>$r[ranah]</td>
    ...
    if ($_SESSION[level] != 'kepala') {
      echo "<td style='width:80px !important'><center>
                                        ...
                                        <a class='btn btn-danger btn-xs alert_notif' title='Delete Data' href='index.php?view=kompetensidasar&hapus=$r[id_kompetensi_dasar]&jdwl=$_GET[id]'><span class='glyphicon glyphicon-remove'></span></a>
                                      </center></td>";
    }
    echo "</tr>";
    $no++;
